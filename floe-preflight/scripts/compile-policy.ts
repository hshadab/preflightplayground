...
// Map a policy slug to the per-policy env var.
const SLUG_TO_ENV_KEY: Record<string, string> = {
  "verifiable-borrow": "NEXT_PUBLIC_POLICY_ID_BORROW",
  "verifiable-x402": "NEXT_PUBLIC_POLICY_ID_X402",
  "verifiable-liquidation": "NEXT_PUBLIC_POLICY_ID_LIQ",
};
...
function writeEnvLocal(slug: string, policyId: string) {
  ...
  // Fallback: if the slug isn't recognised, write to the legacy var so the
...
async function main() {
  ...
  const policyName = policyArgIdx !== -1 ? args[policyArgIdx + 1] : "verifiable-borrow";
